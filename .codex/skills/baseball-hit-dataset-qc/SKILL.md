...
- Use video as the authoritative source for semantic labels. For ground-ball `region`, use the ball's absolute position at its first fielding/control moment: mentally map the fair infield to a top-down fan and divide it from third-base line to first-base line into four equal left-to-right sectors (1, 2, 3, 4). Do not use the fielder's nominal position, player identity, ball path, later throw, or camera orientation as a proxy.
