...
7. Materialize accepted labels into `dataset/` with `scripts/materialize_dataset.py --labels reports/qwen_labels_refined.jsonl --require-audit-pass`.
8. Run validation:
   - `scripts/validate_schema.py`
   - `scripts/validate_media.py`
   - `scripts/detect_contact_audio.py`
   - `scripts/audit_labels.py`
9. Generate an HTML review sheet with `scripts/build_review_sheet.py`.
10. Commit only reviewed batches.
